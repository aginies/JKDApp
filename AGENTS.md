...
## 🚨 CRITICAL RULE: No Single File Over 1,000 Lines

**ABSOLUTE MAXIMUM:** Any single Dart file exceeding **1,000 lines** MUST be immediately refactored into smaller, focused sub-files.
...
- 🟡 **501-1,000 lines**: Refactoring recommended, plan extraction
- 🔴 **1,000+ lines**: IMMEDIATE refactoring required, halt feature development
...
2. **MUST** propose splitting strategy if file > 1,000 lines
...
find lib -name "*.dart" -exec wc -l {} + | awk '$1 > 1000 {print "ERROR: " $2 " has " $1 " lines (max 1000)"; exit 1}'
...
- **Target:** Main screen under **1,000 lines** (compliance achieved)
...
### NEVER DO:
- ❌ Add features to files > 1,000 lines without splitting first
...
- 🚨 Long scroll times to navigate file (>1000 lines visible)
...
### Splitting Strategy (if > 1,000 lines):
...
### Key Principles:
1. **1,000 lines maximum** - Non-negotiable hard limit
...
### Success Metrics:
- Files stay under 1,000 lines (compliance)
