...
class BackupService {
  ...
  static Future<String?> createGlobalBackup(String? galleryPath) async {
    try {
      ...
      final customAnglesFile = File(p.join(docsDir.path, 'custom_kali_angles.json'));
      if (await customAnglesFile.exists()) {
        await customAnglesFile.copy(p.join(workspaceDir.path, 'custom_kali_angles.json'));
      }
  ...
  static Future<bool> restoreGlobalBackup(
    ...
    try {
      ...
      final customAnglesFile = File(p.join(basePath, 'custom_kali_angles.json'));
      if (await customAnglesFile.exists()) {
        final docsDir = await getApplicationDocumentsDirectory();
        await customAnglesFile.copy(p.join(docsDir.path, 'custom_kali_angles.json'));
      }
