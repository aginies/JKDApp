...
void main() {
  group('JkdSeries Editing Tests', () {
    ...
    test('Should handle complex nested move structures (Edit Mode scenarios)', () {
      ...
      final series = JkdSeries(id: 1, title: 'Advanced', moves: [complexMove]);
      ...
      final editedSimultaneous = simultaneousAttack.copyWith(subMoves: [editedMoveA, moveB]);
      final editedComplexMove = complexMove.copyWith(chain: [editedSimultaneous, moveC]);
      
      final editedSeries = series.copyWith(moves: [editedComplexMove]);

      expect(editedSeries.moves[0].chain[0].subMoves[0].name, 'Lead Jab');
      expect(editedSeries.moves[0].chain[1].name, 'Hook'); // Should preserve others
    });
